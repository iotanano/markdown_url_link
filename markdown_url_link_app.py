...
def main():
    ...
    if st.button("Generate Markdown with Links"):
        try:
            ...
            if header_count == 0:
                st.warning("Markdown input must contain at least one header.")  # 2. 如果 markdown 中不包含任何标题，抛出提示。
                return

            if header_count != len(urls):
                st.warning("The number of markdown headers must match the number of URLs.")  # 2. markdown标题与url链接数量不一致，抛出提示
                return
